...
Pattern (see ``TestFirstComeFirstServed.test_two_jobs_processed_in_arrival_order``):
seize the server with a long blocker, pile up jobs behind it, run to completion,
then assert the processing order via ``job.servers_exit_at[server]``.
"""
...
class TestOperationalDueDateIntegration:
    """ODD: jobs leave the queue ordered by their operational due date at s1."""
    ...
    def test_dispatches_lowest_operational_due_date_first(self) -> None:
        ...
        jobs: list[ProductionJob] = []
        # ODD at s1 = t_r + 1 * max(0, (d - t_r) / r), r = 2, t_r = psp_exit (~0).
        # So ODD ~= d / 2 and is fixed at add time (independent of dispatch now).
        # A: 60/2 = 30, B: 40/2 = 20, C: 80/2 = 40. Ascending: B < A < C.
        for sku, due_date in [("A", 60.0), ("B", 40.0), ("C", 80.0)]:
            job = ProductionJob(
                env=env,
                sku=sku,
                servers=[s1, s2],
                processing_times=[1.0, 1.0],
                due_date=due_date,
                priority_policy=operational_due_date,
            )
            sf.add(job)
...
class TestModifiedOperationalDueDateIntegration:
    """MODD: when jobs are late, the now+p term dominates -> SPT-style ordering."""
    ...
    def test_late_jobs_dispatch_by_processing_time(self) -> None:
        ...
        # dispatch -> order by p ascending: B(1) < C(2) < A(3).
...
class TestSlackPerRemainingOperationIntegration:
    """S/OPN: jobs leave the queue ordered by PST divided by remaining op count."""
    ...
    def test_dispatches_lowest_slack_per_operation_first(self) -> None:
        ...
        jobs: list[ProductionJob] = []
        # S/OPN = pst / |R_i|, allowance = 0, now ~= 100, |R_i| = number of servers
        # not yet exited (all of them, since each job is queued at s1).
        # A: routing [2, 3], pst = 200 - 100 - 5 = 95, |R| = 2 -> 47.5.
        # B: routing [1],    pst = 150 - 100 - 1 = 49, |R| = 1 -> 49.0.
        # C: routing [4, 4], pst = 300 - 100 - 8 = 192, |R| = 2 -> 96.0.
        # Ascending: A < B < C.
        for sku, due_date, servers, pts in [
            ("A", 200.0, [s1, s2], [2.0, 3.0]),
            ("B", 150.0, [s1], [1.0]),
            ("C", 300.0, [s1, s2], [4.0, 4.0]),
        ]:
            job = ProductionJob(
                env=env,
                sku=sku,
        ...
        env.run()
        assert _processed_order(jobs, s1) == ["A", "B", "C"]
