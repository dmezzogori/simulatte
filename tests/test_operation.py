...
def test_operation_position_and_equality():
    location = cast(WarehouseLocation, SimpleNamespace(x=3, y=2))
    unit_load = cast(CaseContainer, SimpleNamespace())
    op_a = Operation(unit_load=unit_load, location=location, priority=5)
    op_b = Operation(unit_load=unit_load, location=location, priority=1)
...
def test_input_operation_sets_lift_state():
    location = cast(WarehouseLocation, SimpleNamespace(x=0, y=0))
    unit_load = cast(CaseContainer, SimpleNamespace())
    op = InputOperation(unit_load=unit_load, location=location, priority=0)

    assert op.lift_process is None
    assert not op.lifted.triggered


def test_output_operation_is_subclass():
    location = cast(WarehouseLocation, SimpleNamespace(x=1, y=1))
    unit_load = cast(CaseContainer, SimpleNamespace())
    op = OutputOperation(unit_load=unit_load, location=location, priority=0)
