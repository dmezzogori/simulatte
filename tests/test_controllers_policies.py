...
from simulatte.controllers.agvs_controller import AGVController, idle_feeding_order, least_busy_agv
...
def test_least_busy_agv_chooses_least_busy():
    agv_busy = DummyAGV(kind=AGVKind.FEEDING, n_users=2, n_queue=3)
    agv_free = DummyAGV(kind=AGVKind.FEEDING, n_users=0, n_queue=1)

    chosen = least_busy_agv(agvs=[agv_busy, agv_free])
    assert chosen is agv_free

    chosen_excluding = least_busy_agv(agvs=[agv_busy, agv_free], exceptions=[agv_free])
    assert chosen_excluding is agv_busy


def test_idle_feeding_order_sorts_by_load_and_timestamp():
    ...
    ordered = idle_feeding_order(agvs=[agv_a, agv_b, agv_c])
    assert ordered[0] is agv_c  # fewest users/queue
    assert ordered[-1] is agv_a

    ordered_filtered = idle_feeding_order(agvs=[agv_a, agv_b], exceptions=[agv_a])
...
def test_agv_controller_groups_and_best_selection(capsys, env):
    ...
    controller = AGVController(agvs=agvs, select_agv=lambda agvs, exceptions=None: next(iter(agvs)), env=env)
